...
func TestFrameAuthenticationRejectsPayloadTampering(t *testing.T) {
	key := [32]byte{1, 2, 3}
	wire := SealFrameMAC(&UDPCFrame{
		Magic: UDPC_MAGIC_DEFAULT, Version: UDPC_VERSION, Cmd: CMD_DATA,
		SessionID: 1, PacketNo: 1, Seq: 1, Data: []byte("payload"),
	}, &key)
	wire[UDPC_HDR_SIZE] ^= 0xff
	frame, err := DecodeUDPCFrame(wire, UDPC_MAGIC_DEFAULT)
	if err != nil {
		t.Fatalf("structural parsing should succeed before authentication: %v", err)
	}
	if err := VerifyFrameAuth(frame.Raw(), &key); err == nil {
		t.Fatal("expected payload MAC mismatch")
	}
}
...
func TestNoiseControlFramesCannotHijackSession(t *testing.T) {
	...
	evil := netip.MustParseAddrPort("203.0.113.9:9999")
	...
	rig.sess.processIncomingFrame(ack, evil, 45678)
	if len(rig.sess.unacked) != 1 {
		t.Fatal("cross-IP ACK removed an in-flight frame")
	}
	fin := rig.makeWireFrame(&UDPCFrame{Cmd: CMD_FIN, PacketNo: 2})
	rig.sess.processIncomingFrame(fin, evil, 45678)
	if atomic.LoadInt32(&rig.sess.closed) != 0 {
		t.Fatal("cross-IP FIN closed a Noise session")
	}
	if rig.sess.getRemoteAddr() != rig.clientAddr {
	...
	// A tampered DATA frame must not be able to install the same poisoned path.
	badData := rig.makeWireData(3, 1, []byte("bad"))
	badData.raw[len(badData.raw)-1] ^= 1
	rig.sess.processIncomingFrame(badData, evil, 45678)
	if _, ok := rig.sess.pathAddrs[45678]; ok {
...
func TestNoiseRejectsPoisonedOutOfOrderSlot(t *testing.T) {
	...
	poison := rig.makeWireData(1, 2, []byte("poison"))
	poison.raw[len(poison.raw)-1] ^= 1
	rig.sess.processIncomingFrame(poison, rig.clientAddr, 0)
	if _, ok := rig.sess.recvQueue[2]; ok {
		t.Fatal("undecryptable out-of-order frame entered the reorder queue")
	}
	rig.deliverWireData(rig.makeWireData(2, 2, []byte("second")), rig.clientAddr)
	if _, ok := rig.sess.recvQueue[2]; !ok {
		t.Fatal("authenticated out-of-order frame was not queued")
	}
	rig.deliverWireData(rig.makeWireData(3, 1, []byte("first")), rig.clientAddr)
