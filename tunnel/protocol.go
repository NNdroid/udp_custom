...
// fireAndForgetBufPool recycles send buffers for control frames. They are sent
// synchronously and never retained, so the buffer goes straight back to the
// pool. DATA frames cannot use it — their wire
...
// decryptBufPool recycles plaintext buffers for the AEAD-open of received
// records. The open is synchronous (deliver to target / dispatch to app /
// copy into the reorder queue) and nothing retains the plaintext past the
// enclosing dispatch, so the buffer goes straight back. One buffer covers the
// largest possible plaintext: a full max-size record minus header and tag.
...
func getDecryptBuf() []byte {
	return (*decryptBufPool.Get().(*[]byte))[:]
}

func putDecryptBuf(buf []byte) {
	if cap(buf) == UDPC_MAX_PKT {
		full := buf[:cap(buf)]
		decryptBufPool.Put(&full)
	}
}
