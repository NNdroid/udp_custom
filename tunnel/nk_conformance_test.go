...
// PacketNo-derived nonce scheme (see packetNonce).
...
func TestNK_TransportKeysMatchFlynn(t *testing.T) {
	...
	// Transport nonce starts at 0, which is exactly seqNonce(0). flynn seals
	// with a nil AAD, so we pass nil too — the AEAD format itself must stay
	// interoperable (protocol v2 only ADDS a header AAD at the frame layer).
	payload := []byte("transport message from flynn")
	ct, err := flynnSend.Encrypt(nil, nil, payload)
	if err != nil {
		t.Fatal(err)
	}
	got, err := serverSess.RecvCipher.Decrypt(0, ct, nil)
	if err != nil {
	...
	out := serverSess.SendCipher.Encrypt(0, payload, nil)
...
func TestNK_OurInitiatorWithFlynnResponder(t *testing.T) {
	...
	ct := clientSess.SendCipher.Encrypt(0, payload, nil)
	...
	got2, err := clientSess.RecvCipher.Decrypt(0, ct2, nil)
	if err != nil {
...
func TestNK_ChannelBindingIsSharedAndUnique(t *testing.T) {
	...
	if clientSess.SendCipher.Encrypt(1, []byte("x"), nil)[0] == clientSess2.SendCipher.Encrypt(1, []byte("x"), nil)[0] {
...
func TestNK_OwnRoundTrip(t *testing.T) {
	...
	// Both directions, with out-of-order (PacketNo-derived) nonces.
	for _, seq := range []uint64{3, 1, 2} {
		ct := clientSess.SendCipher.Encrypt(seq, []byte("up"), nil)
		pt, err := serverSess.RecvCipher.Decrypt(seq, ct, nil)
		if err != nil {
	...
	for _, seq := range []uint64{7, 5, 6} {
		ct := serverSess.SendCipher.Encrypt(seq, []byte("down"), nil)
		pt, err := clientSess.RecvCipher.Decrypt(seq, ct, nil)
		if err != nil {
