...
func TestSpreadDialerUsesInjectedListener(t *testing.T) {
	calls := 0
	dialer, err := newSpreadDialer("127.0.0.1:32000-32010", 3, 2, func(network string, laddr *net.UDPAddr) (*net.UDPConn, error) {
		calls++
	...
	defer dialer.Close()
	if calls != 3 {
		t.Fatalf("listener called %d times, want 3", calls)
	}
}
...
func TestSpreadDialerReopenReplacesFailedSocket(t *testing.T) {
	d, err := NewSpreadDialer("127.0.0.1:32000", 1, 1)
	if err != nil {
		t.Fatal(err)
	}
	defer d.Close()
	old := d.Conn(0)
	if old == nil {
		t.Fatal("missing initial socket")
	}
	_ = old.Close()
	replacement, err := d.Reopen(0, old)
	if err != nil {
		t.Fatalf("Reopen: %v", err)
	}
	if replacement == nil || replacement == old {
		t.Fatal("failed socket was not replaced")
	}
	if got := d.Conn(0); got != replacement {
		t.Fatal("replacement was not published to the spread slot")
	}
}
...
func TestSpreadDialerMultiSocketAgainstServer(t *testing.T) {
	...
	ackFrame, serverNonce, _ := parseTestHandshakeACK(t, ackFrame.Raw(), handshakeKeys, clientNonce, false)
	...
	for _, idx := range []int{1, 2, 0} {
		...
		wire := SealFrameAEAD(&UDPCFrame{
			...
			Seq:       dataSeq,
			Data:      payload,
		}, frameKeys.Send, payload)
		packetNo++
		if err := d.SendAt(idx, wire); err != nil {
