...
func (r *testRig) deliverWireData(frame *UDPCFrame, addr netip.AddrPort) bool {
	r.t.Helper()
	return r.sess.processIncomingFrame(frame, addr, 0)
}

func (r *testRig) recvFrame(timeout time.Duration) *UDPCFrame {
	return r.recvFrameFrom(r.client, timeout)
}

func (r *testRig) recvFrameFrom(conn *net.UDPConn, timeout time.Duration) *UDPCFrame {
	r.t.Helper()
...
func (r *testRig) expectNoFrame(timeout time.Duration) {
	r.expectNoFrameFrom(r.client, timeout)
}

func (r *testRig) expectNoFrameFrom(conn *net.UDPConn, timeout time.Duration) {
	r.t.Helper()
	conn.SetReadDeadline(time.Now().Add(timeout))
	buf := make([]byte, 2048)
...
// A new IP cannot move the session until it proves return-path reachability.
// The rejected DATA keeps its packet number reusable so ARQ can deliver it
// after validation.
func TestMigrationViaAuthenticatedDataRequiresPathValidation(t *testing.T) {
	rig := newTestRig(t, true)
	// A second loopback IP simulates a different client IP. Some CI containers
	// only assign 127.0.0.1 to lo (no /8), so skip when the host cannot bind
	// extra loopback addresses — the migration logic itself is IP-agnostic.
	newClient, err := net.ListenUDP("udp4", &net.UDPAddr{IP: net.ParseIP("127.0.0.2"), Port: 0})
	if err != nil {
		t.Skipf("loopback 127.0.0.2 unavailable on this host: %v", err)
	}
	defer newClient.Close()
	newPath := newClient.LocalAddr().(*net.UDPAddr).AddrPort()
	...
	if rig.deliverWireData(bad, newPath) {
	...
	rig.expectNoFrameFrom(newClient, 150*time.Millisecond)

	if rig.deliverWireData(good, newPath) {
		t.Fatal("authenticated DATA was delivered before path validation")
	}
	challenge := rig.recvFrameFrom(newClient, time.Second)
	if challenge.Cmd != CMD_PATH_CHALLENGE {
		t.Fatalf("challenge command = %d", challenge.Cmd)
	}
	response := rig.makeWireFrame(&UDPCFrame{
		Cmd: CMD_PATH_RESPONSE, PacketNo: 2, Data: challenge.Data,
	})
	if !rig.sess.processIncomingFrame(response, newPath, 0) {
		t.Fatal("valid path response was rejected")
	}
	if !rig.deliverWireData(good, newPath) {
		t.Fatal("DATA retransmission was rejected after path validation")
	}
	if got := string(rig.waitForTargetBytes(4, time.Second)); got != "AAAA" {
		t.Fatalf("target got %q", got)
	}
	if got := rig.sess.getRemoteAddr(); got != newPath {
		t.Fatalf("validated DATA path = %s, want %s", got, newPath)
	}
}

func TestAuthenticatedPingMigratesIdleSession(t *testing.T) {
	rig := newTestRig(t, false)
	// Same skip rationale as above: extra loopback IPs are a host capability.
	newClient, err := net.ListenUDP("udp4", &net.UDPAddr{IP: net.ParseIP("127.0.0.2"), Port: 0})
	if err != nil {
		t.Skipf("loopback 127.0.0.2 unavailable on this host: %v", err)
	}
	...
	if rig.sess.processIncomingFrame(ping, newPath, 0) {
		t.Fatal("new path was accepted before reachability validation")
	}
	if got := rig.sess.getRemoteAddr(); got == newPath {
		t.Fatal("PING migrated the path before challenge response")
	}
	challenge := rig.recvFrameFrom(newClient, time.Second)
	if challenge.Cmd != CMD_PATH_CHALLENGE || len(challenge.Data) != pathChallengeSize {
		t.Fatalf("path challenge = cmd %d payload %x", challenge.Cmd, challenge.Data)
	}
	secondPing := rig.makeWireFrame(&UDPCFrame{Cmd: CMD_PING, PacketNo: 2})
	if rig.sess.processIncomingFrame(secondPing, newPath, 0) {
		t.Fatal("second PING was accepted before reachability validation")
	}
	secondChallenge := rig.recvFrameFrom(newClient, time.Second)
	if !bytes.Equal(secondChallenge.Data, challenge.Data) {
		t.Fatal("repeated traffic replaced an unexpired path challenge token")
	}
	response := rig.makeWireFrame(&UDPCFrame{
		Cmd: CMD_PATH_RESPONSE, PacketNo: 3, Data: challenge.Data,
	})
	if !rig.sess.processIncomingFrame(response, newPath, 0) {
		t.Fatal("valid path response was rejected")
	}
	if got := rig.sess.getRemoteAddr(); got != newPath {
		t.Fatalf("validated idle session path = %s, want %s", got, newPath)
	}
	if rig.sess.processIncomingFrame(ping, newPath, 0) {
		t.Fatal("pre-validation PING replay was accepted after migration")
	}
	rig.expectNoFrameFrom(newClient, 50*time.Millisecond)
}

func TestUnauthenticatedPingCannotMigrateSession(t *testing.T) {
	rig := newTestRig(t, false)
	oldPath := rig.sess.getRemoteAddr()
	newPath := netip.MustParseAddrPort("203.0.113.20:45679")
	ping := rig.makeWireFrame(&UDPCFrame{Cmd: CMD_PING, PacketNo: 1})
	ping.Raw()[len(ping.Raw())-1] ^= 0x80
	if rig.sess.processIncomingFrame(ping, newPath, 0) {
		t.Fatal("tampered PING was accepted")
	}
	if got := rig.sess.getRemoteAddr(); got != oldPath {
		t.Fatalf("unauthenticated PING migrated path to %s", got)
	}
}
...
func TestUpdateRemoteAddrPolicy(t *testing.T) {
	...
	portBump := netip.AddrPortFrom(rig.clientAddr.Addr(), rig.clientAddr.Port()+1)
	...
	// Different IP without completed path validation: rejected.
	evil := netip.MustParseAddrPort("203.0.113.9:1")
	sess.updateRemoteAddr(evil, false)
	if sess.getRemoteAddr().String() != portBump.String() {
		t.Fatal("unvalidated IP change must be rejected")
	}

	// Different IP after validation: accepted.
	sess.updateRemoteAddr(evil, true)
	if sess.getRemoteAddr().String() != evil.String() {
		t.Fatal("validated IP change must be accepted")
	}
}
