...
// We therefore derive the nonce from the frame PacketNo — nonce = LE64(PacketNo) — which
// is exactly the QUIC/TLS record-protection pattern (RFC 9001 §5.3 derives a
// per-packet nonce from the packet number). Keys, cipher and transcript are
// unchanged; only the nonce source differs, and protocol v2 additionally uses
// the frame header as AAD.
const (
...
// packet number. Retransmissions reuse the complete encoded frame and therefore
// reuse only the exact same nonce/ciphertext pair.
//
...
// SealFrameAEAD protects one complete Noise session record. The encrypted
...
func SealFrameAEAD(f *UDPCFrame, c *AEADCipherState, plaintext []byte) []byte {
	...
	total := UDPC_HDR_SIZE + len(plaintext) + chacha20poly1305.Overhead
	if total > UDPC_MAX_PKT {
		return nil
	}
	wire := make([]byte, total)
	n := sealFrameAEADInto(wire, f, c, plaintext)
	return wire[:n]
}
...
func sealFrameAEADInto(wire []byte, f *UDPCFrame, c *AEADCipherState, plaintext []byte) int {
	...
	sealed := c.aead.Seal(wire[UDPC_HDR_SIZE:UDPC_HDR_SIZE], nonce[:], plaintext, header)
	return UDPC_HDR_SIZE + len(sealed)
}
...
func OpenFrameAEADInto(dst []byte, f *UDPCFrame, c *AEADCipherState) ([]byte, error) {
	...
	nonce := packetNonce(f.PacketNo)
	protected := f.raw[UDPC_HDR_SIZE:]
	return c.aead.Open(dst[:0], nonce[:], protected, f.raw[:UDPC_HDR_SIZE])
}
