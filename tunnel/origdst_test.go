...
func TestReadWithOrigDst_NoAncillaryData(t *testing.T) {
	...
	if !from.IsValid() || from != netip.AddrPortFrom(netip.MustParseAddr("127.0.0.1"), uint16(cli.LocalAddr().(*net.UDPAddr).Port)) {
		t.Fatalf("peer = %v, want 127.0.0.1:%d", from, cli.LocalAddr().(*net.UDPAddr).Port)
	}
	// IP_RECVORIGDSTADDR delivers an ORIGDSTADDR cmsg for EVERY received
	// datagram, reporting the destination from the IP header — DNAT or not.
	// Without DNAT that destination is the socket's own bound port, so the
	// parser must return exactly that. (This is the end-to-end recvmsg+parse
	// pipeline check; the pre-DNAT mirroring case cannot be exercised without
	// netfilter rules.)
	wantPort := srv.LocalAddr().(*net.UDPAddr).Port
	if origDstPort != wantPort {
		t.Fatalf("origDstPort = %d, want %d (the packet's actual destination)", origDstPort, wantPort)
	}
}
