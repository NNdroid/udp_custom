...
- **Network-Change Recovery**: a client receive loop supervises every spread socket, re-resolves hostnames and rebuilds failed UDP sockets with capped backoff. Any session migration to a new IP must complete an encrypted bidirectional path challenge first; DATA waits for validation and is then recovered by normal ARQ retransmission.
...
there is no downgrade or legacy parser. The exact wire format is documented in
the maintained wire-spec document (kept out of version control).
...
To opt into replacement sessions, use `AutoReconnect` instead of the direct
dial. After a terminal session error, the next operation lazily establishes a
new tunnel; retry the failed request at an application boundary:

```go
resilient := tunnel.NewAutoReconnect(cli, tunnel.DialOptions{Target: "tcp://127.0.0.1:2222"}, nil)
...
- `NewAutoReconnect` preserves deadlines and granted-target callbacks across
  replacement sessions. It never replays a failed byte-stream operation because
  the remote TCP stream cannot be resumed safely.
...
- Wire compatibility with the Stun Android/TV client follows the v2 wire-spec
  document (kept out of version control); keep both sides on v2.
