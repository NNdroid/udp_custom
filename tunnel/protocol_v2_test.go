...
func TestProtocolV2EveryControlFrameIsAuthenticated(t *testing.T) {
	...
	for i, cmd := range []uint8{CMD_PATH_CHALLENGE, CMD_PATH_RESPONSE} {
		t.Run(commandName(cmd), func(t *testing.T) {
			...
			frame, err := DecodeUDPCFrame(wire, UDPC_MAGIC_DEFAULT)
			if err != nil || !validSessionFrameShape(frame) {
				t.Fatalf("path-control frame rejected: %v", err)
			}
			plain, err := OpenFrameAEAD(frame, cipher)
	...
	for _, n := range []int{0, pathChallengeSize - 1, pathChallengeSize + 1} {
		frame := &UDPCFrame{
			Magic: UDPC_MAGIC_DEFAULT, Version: UDPC_VERSION,
			Cmd: CMD_PATH_RESPONSE, SessionID: 99, PacketNo: 20,
			Data: make([]byte, n),
		}
		if validSessionFrameShape(frame) {
