...
//
// NOTE: release-to-rebind is a race window — under -race/CI load the OS (or a
// parallel process) can hand a reserved port to another socket before the code
// under test binds it. Tests that require a SPECIFIC port to be bindable must
// verify the bind succeeded and skip/adjust otherwise (see
// TestMultipathSession_RoutesReplyPerPath).
...
// requireBindablePort reserves port, releases it, and verifies the code under
// test can actually re-bind it. Returns false when the OS handed the port to
// someone else in the release window — the caller should Skip rather than
// report a pool bug for environmental port theft.
func requireBindablePort(t *testing.T, p *sendSockPool, port int) bool {
	t.Helper()
	c, err := p.Get(port)
	if err != nil {
		return false
	}
	gotPort := c.LocalAddr().(*net.UDPAddr).Port
	if gotPort != port {
		return false
	}
	return true
...
func TestReplyFromOrigPort_UsesPerPortSocket(t *testing.T) {
	...
	srv.replyFromOrigPort(replyPort, client.LocalAddr().(*net.UDPAddr).AddrPort(), payload)
...
func TestReplyFromOrigPort_FallsBackToMain(t *testing.T) {
	...
	srv.replyFromOrigPort(0, client.LocalAddr().(*net.UDPAddr).AddrPort(), payload) // origPort 0 -> main socket
...
func TestMultipathSession_RoutesReplyPerPath(t *testing.T) {
	...
	// release-to-bind window (CI load widens it). If the pool cannot bind
	// them, the reply-source assertions below would be environmental noise —
	// skip instead of failing.
	for _, port := range []int{p1, p2} {
		if !requireBindablePort(t, srv.sockPool, port) {
	...
	sess.setPath(p2, client2.LocalAddr().(*net.UDPAddr).AddrPort())
