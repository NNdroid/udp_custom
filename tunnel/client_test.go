...
func TestClientRebuildsFailedSpreadSocket(t *testing.T) {
	tn := startTunnel(t, "127.0.0.1:0", [32]byte{}, false, 1)
	defer tn.cleanup()

	old := tn.client.dialer.Conn(0)
	if old == nil {
		t.Fatal("missing client spread socket")
	}
	_ = old.Close()
	deadline := time.Now().Add(3 * time.Second)
	for tn.client.dialer.Conn(0) == old && time.Now().Before(deadline) {
		time.Sleep(10 * time.Millisecond)
	}
	if tn.client.dialer.Conn(0) == old {
		t.Fatal("receive supervisor did not publish a replacement socket")
	}
	...
	if _, err := io.ReadFull(conn, got); err != nil {
		t.Fatalf("echo through replacement socket: %v", err)
	}
