...
func TestNoiseEncryptDecryptRoundTrip(t *testing.T) {
	...
	for _, seq := range []uint32{1, 2, 42, 0xFFFFFFFF, 0x7FFFFFFF} {
		ct := send.Encrypt(seq, plain)
		got, err := recv.Decrypt(seq, ct)
		if err != nil {
...
func TestNoiseDecryptOutOfOrder(t *testing.T) {
	...
	cts := make(map[uint32][]byte, 5)
	for seq := uint32(5); seq <= 9; seq++ {
		cts[seq] = send.Encrypt(seq, plain)
	}
	// Decrypt in arrival order 9,5,7,6,8 — every one must open.
	for _, seq := range []uint32{9, 5, 7, 6, 8} {
		got, err := recv.Decrypt(seq, cts[seq])
		if err != nil {
...
func TestNoiseDecryptRetransmission(t *testing.T) {
	...
	ct := send.Encrypt(7, plain)

	for i := 0; i < 3; i++ {
		got, err := recv.Decrypt(7, ct)
		if err != nil {
...
// A frame opened under the wrong seq must fail (that is what makes replay of a
// frame into a different slot detectable) and must not corrupt later frames.
func TestNoiseDecryptWrongSeqFails(t *testing.T) {
	send, recv, _ := newTestCipherStates(t)
	ct := send.Encrypt(5, []byte("payload"))
	if _, err := recv.Decrypt(6, ct); err == nil {
		t.Fatal("decrypt under wrong seq should fail")
	}
	// The failed attempt must not have consumed anything: seq 5 still opens.
	if got, err := recv.Decrypt(5, ct); err != nil || string(got) != "payload" {
...
func TestNoiseDecryptTamperedFails(t *testing.T) {
	send, recv, _ := newTestCipherStates(t)
	ct := send.Encrypt(3, []byte("authenticate me"))
	ct[len(ct)-1] ^= 0x01
	if _, err := recv.Decrypt(3, ct); err == nil {
...
func TestNoiseKeySeparation(t *testing.T) {
	...
	ct := clientSend.Encrypt(1, []byte("c2s"))
	if got, err := serverRecv.Decrypt(1, ct); err != nil || string(got) != "c2s" {
		t.Fatalf("c2s path broken: %v", err)
	}
	if _, err := clientRecv.Decrypt(1, ct); err == nil {
		t.Fatal("c2s ciphertext must not open under the s2c key")
	}

	ct2 := serverSend.Encrypt(1, []byte("s2c"))
	if got, err := clientRecv.Decrypt(1, ct2); err != nil || string(got) != "s2c" {
...
func TestNoiseSessionAgreement(t *testing.T) {
	...
	ct := client.SendCipher.Encrypt(1, []byte("hello from client"))
	got, err := server.RecvCipher.Decrypt(1, ct)
	if err != nil || string(got) != "hello from client" {
		t.Fatalf("client->server failed: %v", err)
	}

	ct2 := server.SendCipher.Encrypt(1, []byte("hello from server"))
	got2, err := client.RecvCipher.Decrypt(1, ct2)
